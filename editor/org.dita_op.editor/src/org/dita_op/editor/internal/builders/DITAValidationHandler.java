...
public class DITAValidationHandler extends DefaultHandler2 {
	...
	public void startElement(String uri, String localname, String qName,
		...
		if (href != null) {
			// XXX: Checks only references for which the scope is local because
			// the target may not be available at the time of the validation for
			// any other type of scope.
			...
			if (scope == null || scope.equals("local")) { //$NON-NLS-1$
				try {
					...
					onError(Messages.getString(
							"DITAValidationHandler.invalidURL", href)); //$NON-NLS-1$
	...
	private void validateReference(String href) throws IOException,
			SAXException, URISyntaxException {
		InputSource target = resolveEntity("href", null, //$NON-NLS-1$
