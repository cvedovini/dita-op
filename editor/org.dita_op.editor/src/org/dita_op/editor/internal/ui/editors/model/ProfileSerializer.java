...
public class ProfileSerializer {
	...
	private void serialize(Val val) throws SAXException {
		...
		if (val.getBackgroundConflictColor() != null
			...
			if (val.getForegroundConflictColor() != null) {
				atts.addAttribute(EMPTY, EMPTY, "foreground-conflict-color", //$NON-NLS-1$
						CDATA,
						RGBUtils.toString(val.getForegroundConflictColor()));
			}

			if (val.getBackgroundConflictColor() != null) {
				atts.addAttribute(EMPTY, EMPTY, "background-conflict-color", //$NON-NLS-1$
						CDATA,
	...
	private void serializeFlag(String name, Flag f) throws SAXException {
		if (f != null) {
			AttributesImpl atts = new AttributesImpl();
			atts.addAttribute(EMPTY, EMPTY, "imageref", CDATA, f.getImageRef()); //$NON-NLS-1$
			...
			if (alttext != null) {
				...
				handler.characters(alttext.toCharArray(), 0, alttext.length());
