...
import org.dita_op.editor.internal.ui.editors.model.Flag;
...
public class FlagSection {
	...
	protected void createClientArea(Composite container, FormToolkit toolkit) {
		...
		toolkit.createLabel(container, Messages.getString("FlagSection.imageref.label")); //$NON-NLS-1$
		...
		Button browseButton = toolkit.createButton(container, Messages.getString("FlagSection.browse.button"), SWT.NONE); //$NON-NLS-1$
		...
		toolkit.createLabel(container, Messages.getString("FlagSection.alttext.label")); //$NON-NLS-1$
	...
	public Flag getFlag() {
		...
		if (imageRef.length() > 0) {
			...
			String altText = altTextText.getText();
